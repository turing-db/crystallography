...
Nine node labels and eleven edge types.
...
**Useful and easy to miss:** `SKIP`, `IS NOT NULL` is out but comparisons are
fine, arithmetic in `RETURN` (`RETURN r.length * 2`), `toInteger()` /
`toFloat()`, `labels(n)`, `count(*)`, multi-key `ORDER BY`, and
`cosine_similarity()` / `euclidean_distance()` over embedding properties.
...
Or ask for one directly:

```cypher
MATCH (s:Structure)-[:IN_SPACE_GROUP]->(g:SpaceGroup)
WHERE g.hm_symbol = 'P 1 21/c 1' RETURN count(s)
```

Note this graph keys `SpaceGroup` by group **number**, so P2₁/c covers all
settings of No. 14. `cod_versioned` keys by Hermann-Mauguin **symbol**, where
they are separate nodes. Both are deliberate; say which you are using.
...
`a1.cod_id = a2.cod_id` is not optional. `Component` nodes are deduplicated
corpus-wide, so without it the query will happily join a bond in one COD entry
to a bond in another and call the pair a dimer.
...
admitted as well): `0` finite motif, `1` chain, `2` sheet, `3` framework, `-1`
no qualifying contact, `-2` declined.
...
That one returns **82 structures across five different space-group settings** —
...
- **Commit hashes are build-time values.** Rebuilding the same graph from the
  ...
  runtime from the `cod_versions` ledger, or from `CALL db.history()`.
...
- Filter on `kind` and `h_inferred` early — they are indexed and highly
  selective.
...
Two gates run every query the project ships against a live server, and both fail
loudly rather than returning empty:
...
The second also checks that every `const [...] = await Promise.all([...])` in
the front end names exactly as many variables as the array has entries — the
panels read their results positionally, so a mismatch renders a plausible wrong
