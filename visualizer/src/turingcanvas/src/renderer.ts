...
export class TuringRenderer {
  ...
  // Set the flowing-energy animation strength (0 = off). Driven by Grid Studio.
  setFlow(strength: number) {
    const u = this.edgeMaterial.uniforms.uFlowStrength
  ...
  update() {
    ...
    // Advance the flowing-energy clock only while the effect is enabled.
    const flow = this.edgeMaterial.uniforms.uFlowStrength
    if (flow && flow.value > 0) {
      const t = this.edgeMaterial.uniforms.uFlowTime
  ...
  setMvps(nodes: TuringNode[], edges: TuringEdge[], edgeScale: number) {
    ...
    edges.forEach((e, i) => {
      ...
      // Grid Studio can pin a per-edge state colour (utilisation / tripped).
      // When present it overrides the default and reads at full strength.
      const gridColor = e.gridColor
      if (gridColor !== undefined) {
        color = new THREE.Color(gridColor)
        baseScale.y = e.gridWidth ?? 0.14
      ...
      // per-edge flow speed (0 = stopped). Grid Studio drives e.flowRate.
      this.edgeMeshes[bufIndex].setUniformAt('uFlowRate', edgeIndex, e.flowRate ?? 1.0)
      // per-edge draw progress for the grow-from-source reveal (1 = full).
      this.edgeMeshes[bufIndex].setUniformAt('uDraw', edgeIndex, e.gridDraw ?? 1.0)
