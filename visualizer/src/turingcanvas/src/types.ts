...
export type TuringEdge = {
  ...
  target: TuringNode
  data?: NodeData
  // Optional per-edge state colour (Grid Studio). When set, setMvps paints the
  // edge this colour instead of the default; clear it to restore normal render.
  gridColor?: number
  // Optional per-edge flow-animation speed (Grid Studio). 0 = stopped.
  flowRate?: number
  // Optional per-edge thickness (Grid Studio). Overrides the default when set.
  gridWidth?: number
  // Optional per-edge draw progress [0..1] for a grow-from-source reveal. When
  // < 1, the edge is only drawn up to this fraction of its length; undefined or
  // 1 means fully drawn (default). Animated by instance.animateEdgeDraw().
  gridDraw?: number
}
