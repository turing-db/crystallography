...
export class TuringInstance {
  ...
  // Pin (or clear, with undefined) a per-edge state colour. setMvps applies it
  // on the next frame, so no explicit redraw is needed here.
  setEdgeColor(e: TuringEdge, color: number | undefined) {
    e.gridColor = color
  }

  // Enable/disable the flowing-energy edge animation (0 = off).
  setFlow(strength: number) {
    this.renderer.setFlow(strength)
  }

  // Per-edge flow speed (0 = stopped). Applied by setMvps each frame.
  setEdgeFlow(e: TuringEdge, rate: number) {
    e.flowRate = rate
  }

  // Per-edge thickness override (Grid Studio). Applied by setMvps each frame.
  setEdgeWidth(e: TuringEdge, width: number | undefined) {
    e.gridWidth = width
  }

  // Per-edge draw progress [0..1]; setMvps clips the edge to this fraction.
  setEdgeDraw(e: TuringEdge, v: number | undefined) {
    e.gridDraw = v
  }

  // Animate an edge growing out of its source node toward the target over
  // `durationMs`. Runs on its own rAF so it doesn't block callers; setMvps
  // reads e.gridDraw every frame, so this simply drives that value 0 → 1.
  animateEdgeDraw(e: TuringEdge, durationMs = 160) {
    const start = performance.now()
    e.gridDraw = 0
    const tick = () => {
      const p = Math.min(1, (performance.now() - start) / Math.max(1, durationMs))
      e.gridDraw = p
  ...
  // Project a node's world position to canvas pixel coordinates (top-left
  // origin), or null if the node isn't present. Used to anchor overlays.
  projectNode(id: number): { x: number; y: number } | null {
    const n = this.nodeMap.get(id)
    if (!n) return null
    const v = new THREE.Vector3(n.x, n.y, 0).project(this.renderer.camera)
    const w = this.renderer.width
    const h = this.renderer.height
    // Invert the raycaster's screen→NDC mapping (events.ts):
    //   ndc.x = offsetX/w*2 - 1 ; ndc.y = -(offsetY/h)*2 + 1
  ...
  // Frame the camera on a specific subset of nodes (used to focus a traversal
  // subgraph within a large graph). Zooms in (cap 6) unlike fitView.
  fitNodes(ids: number[], padding = 1.8) {
    let minX = Infinity
    let maxX = -Infinity
    let minY = Infinity
    let maxY = -Infinity
    ...
    const zoom = Math.min(viewWidth / (width * padding), viewHeight / (height * padding), 6)
